...
////OBJETOS INSTANCIADOS PARA USO NESTA CLASSE
	...
	public void setValTableSAC() {
	    ...
	    for (int i = 0; i < emprest.getQtde_parcela(); i++) {
	        data[i][2] = x.format(EmprestControl.calcAmortSac(emprest.getQtde_parcela(), emprestcontrol.getSalDev()));
	    }
	    ...
	    for (int i = 0; i < emprest.getQtde_parcela(); i++) {
	        if (i <= 0) {
	            ...
	            data[i][3] = x.format(EmprestControl.calcJurosSac(emprest.getVl_emprest(), emprestcontrol.getJuros()));
	            data[i][1] = x.format(EmprestControl.calcSacPMT(EmprestControl.calcAmortSac(emprest.getQtde_parcela(), emprest.getVl_emprest()), EmprestControl.calcJurosSac(emprest.getVl_emprest(), emprestcontrol.getJuros())));
	            pmt = (float) EmprestControl.calcSacPMT(EmprestControl.calcAmortSac(emprest.getQtde_parcela(), emprest.getVl_emprest()), EmprestControl.calcJurosSac(emprest.getVl_emprest(), emprestcontrol.getJuros()));
